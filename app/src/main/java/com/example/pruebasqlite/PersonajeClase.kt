...
class Mochila(private var pesoMochila: Int){
    ...
    fun getNumeroArticulos(db:DatabaseHelper):String{
        return db.consultaLecturaBBDD("COUNT SELECT * FROM articulos")
    }
    /*
    fun addArticulo(db:DatabaseHelper,articulo: Articulo) {
                ...
                Articulo.TipoArticulo.ARMA -> {
                    when (articulo.getNombre()) {
                        ...
                        Articulo.Nombre.MARTILLO, Articulo.Nombre.GARRAS -> {
                            db.consultaEscrituraBBDD("")
                            this.pesoMochila -= articulo.getPeso()
                            println("${articulo.getNombre()} ha sido añadido a la bbdd.")
                        }
    ...
    */
    ...
    fun getContenido(db:DatabaseHelper): String {
        return db.consultaLecturaBBDD("SELECT * FROM articulos")
    }
    fun findObjeto(db:DatabaseHelper,nombre: Articulo.Nombre): String {
        return db.consultaLecturaBBDD("SELECT * from articulos where nombre = $nombre").toString()
    }
    /*
    override fun toString(): String {
